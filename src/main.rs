...
use iced::{Element, Font, Pixels, Task};
...
fn fetch_save_figma_file(file_key: &str, access_token: &str, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    for node in root.iter_mut() {
                        if let Some(found_child) = node.find_child(path.clone()) {
                            if msg == NodeMessage::Select {
                                if let Some(figma_node) = find_figma_node(&self.figma_file.clone().unwrap().clone()
